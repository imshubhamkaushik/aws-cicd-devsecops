...
# 1. Base Security Groups (NO inline rules)
resource "aws_security_group" "jenkins" {
  name        = "jenkins-sg"
  description = "Security group for Jenkins"
  vpc_id      = aws_vpc.this.id
...
# 2. Jenkins SG Rules
resource "aws_security_group_rule" "jenkins_allow_ssh_me" {
  description       = "SSH for Ansible provisioning"
  type              = "ingress"
  from_port         = 22
  to_port           = 22
...
resource "aws_security_group_rule" "jenkins_allow_8080_me" {
  description       = "Jenkins UI and Jenkins webhook"
  type              = "ingress"
  from_port         = 8080
  to_port           = 8080
...
# 3. SonarQube SG Rules
resource "aws_security_group_rule" "sonar_allow_ssh_me" {
  description       = "SSH for Ansible provisioning"
  type              = "ingress"
  from_port         = 22
  to_port           = 22
...
resource "aws_security_group_rule" "sonar_allow_9000_me" {
  description       = "SonarQube UI - admin browser access via SSH tunnel"
  type              = "ingress"
  from_port         = 9000
  to_port           = 9000
